...
export default function BookList({ rating }: { rating: string }) {
  const [books, setBooks] = React.useState<IBook[]>([]);
  const router = useRouter();
  React.useEffect(() => {
    const fetchBooks = async () => {
      try {
        const response = await axios.get(`/closed/books/ratingRange?minRating=${rating}&maxRating=5.0`);
        setBooks(response.data.books);
      } catch (error) {
        console.error(`Error fetching book-list /closed/books/ratingRange?minRating=${rating}`, error);
      }
    };
    fetchBooks();
  }, []);
